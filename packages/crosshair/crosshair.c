...
static gboolean on_draw(GtkWidget *w, cairo_t *cr, gpointer _) {
  ...
  cairo_arc(cr, a.width / 2.0, a.height / 2.0, customRadius, 0, 2 * G_PI);
  cairo_fill(cr);
  return FALSE;
}
...
static void activate(GtkApplication *app, gpointer _) {
  ...
  gtk_layer_set_exclusive_zone(GTK_WINDOW(win), is_fullscreen ? -1 : 1);
  ...
  gdk_window_input_shape_combine_region(gtk_widget_get_window(win), empty, 0,
                                        0);
  cairo_region_destroy(empty);
}

int main(int argc, char **argv) {
  is_fullscreen = (argc >= 2) ? (strcmp(argv[1], "f") == 0) : false;
  ...
  GtkApplication *app =
      gtk_application_new("se.n1k0.crosshair", G_APPLICATION_DEFAULT_FLAGS);
